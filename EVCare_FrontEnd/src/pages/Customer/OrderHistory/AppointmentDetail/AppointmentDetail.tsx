...
export default function AppointmentDetail({ onClose, open }: Props) {
  if (!open) return;
      ...
      <Wrapper isOpen={open}>
        ...
        <OrderModal isOpen={open} onClick={(e) => e.stopPropagation()}>
          ...
          >
            ...
            <Status>
              Status: <StatusBadge status={orderStatus}>{orderStatus}</StatusBadge>
            </Status>
          </Row>
          <ModalContent>
            {/* Customer + Staff Info */}
            <Section>
              ...
              <Row>
                <NameBoxComponent label="Customer's Name" name={customer.name} />
                <NameBoxComponent label="Service Staff" name={staffs[0].name} />
              </Row>
              <Row>
                <NameBoxComponent label="Vehicle Model" name={customer.vehicle_model} />
            ...
            <Section>
              {orderStatus === "cancel" ? (
                <>
                  <Title>Reason</Title>
                  <NoteBox readOnly value={customer.reason} placeholder="Reason" />
                </>
              ) : (
                <>
                  <Title>Service</Title>
                  <ServiceList>
                    {services.map((s) => (
                      <Row key={s.id}>
                        ...
                        <ServicePrice>{s.price.toLocaleString("vi-VN")}VNĐ</ServicePrice>
                      </Row>
                    ))}
                  </ServiceList>
